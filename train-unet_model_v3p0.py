...
from unet_model_v3p0 import unet
...
feature_data          = '/glade/work/hardt/ds612/2000-2013_June-Sept_DS128_QR_' + str(ALT) + 'm_AGL.nc'
label_data            = '/glade/work/hardt/ds612/2000-2013_June-Sept_DS128_W_' + str(ALT) + 'm_AGL.nc'
...
#
train_dataset = tf.data.Dataset.from_tensor_slices((feature[train_data_start:train_data_end,:,:,np.newaxis], label[train_data_start:train_data_end,:,:,np.newaxis]))
val_dataset   = tf.data.Dataset.from_tensor_slices((feature[val_data_start:val_data_end,:,:,np.newaxis], label[val_data_start:val_data_end,:,:,np.newaxis]))
...
mse = tf.keras.losses.MeanSquaredError()
#model.compile(optimizer = SGD(lr=1e-4, momentum=0.5), loss=cust_loss(0.01), metrics = ['accuracy'], run_eagerly=True)
model.compile(optimizer = Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1.0, decay=0.0), loss=cust_loss(0.01), metrics = ['mae'], run_eagerly=True)
