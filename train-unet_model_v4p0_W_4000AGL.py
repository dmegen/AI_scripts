...
wandb.init(
  project="Predicting-W",
  name="14Chan-W-4km 100-300 epochs",
...
previous_model = 'trained_model_feature-0to6.5km_at_500m_steps_label-4000m_2020_10_16_18_41.h5'
...
train_data_start = 0
train_data_end   = int( num_images * data_fraction_for_training  / config.batch_size ) * config.batch_size

val_data_start = train_data_end + 1
val_data_end = int(  ( num_images * (data_fraction_for_training + data_fraction_for_validation) - val_data_start)  / config.batch_size )
val_data_end = (val_data_end * config.batch_size) + val_data_start
...
#
#
train_dataset = train_dataset.shuffle(SHUFFLE_BUFFER_SIZE, reshuffle_each_iteration=True).batch(config.batch_size, drop_remainder=True)
...
adam = Adam(lr=config.learn_rate, beta_1=config.beta_1, beta_2=config.beta_2, epsilon=config.epsilon, decay=config.decay)

model.compile(optimizer=config.optimizer, loss=cust_loss(0.01), metrics = ['accuracy','mae'], run_eagerly=True)
...
# do the training
#
#model.fit(train_dataset, epochs=100, validation_data=val_dataset, callbacks=[tensorboard_callback, LRS, checkpoint])image
model.fit(train_dataset, epochs=config.epochs, validation_data=val_dataset, callbacks=[tensorboard_callback, checkpoint, WandbCallback()])
